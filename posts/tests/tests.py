...
class MentionsHelperTests(TestCase):
    def setUp(self):
        client = Client()
        john = User.objects.create_user(username="John", password="newpass1234")
        bill = User.objects.create_user(username="Bill", password="newpass1234")
        john_profile = UserProfileInfo.objects.create(user=john)
        input_str = "some string with a @John mentioned"
        output_str = "some string with a [@John](/accounts/profile/John) mentioned"

    def test_check_for_mentions_detect_username(self):
        input_str = "some string with a @John mentioned"
        output_str = "some string with a [@John](/accounts/profile/John) mentioned"
        new_str = mentions.check_for_mentions(input_str)
        self.assertEqual(new_str, output_str)

    def test_check_for_mention_handle_multiple_usernames(self):
        input_str = "some string with a @John and @Bill mentioned"
        output_str = "some string with a [@John](/accounts/profile/John) and [@Bill](/accounts/profile/Bill) mentioned"
        new_str = mentions.check_for_mentions(input_str)
        self.assertEqual(new_str, output_str)

    def test_check_for_mention_handle_no_usernames(self):
        input_str = "some string with a John mentioned"
        new_str = mentions.check_for_mentions(input_str)
        self.assertEqual(new_str, input_str)

    def test_check_for_mention_user_dont_exist(self):
        input_str = "some string with a @Andy mentioned"
        new_str = mentions.check_for_mentions(input_str)
        self.assertEqual(new_str, input_str)

    def test_urlify_mention_handle_empty_username(self):
        input_username = "@"
        url_str = mentions.urlify_mention(input_username)
        self.assertEqual(input_username, url_str)

    def test_urlify_mention_make_markdown_url(self):
        input_username = "@John"
        url_str = mentions.urlify_mention(input_username)
        self.assertEqual("[@John](/accounts/profile/John)", url_str)